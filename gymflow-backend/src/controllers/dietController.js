// ── GET /api/diet/meals ───────────────────────────────────────────────────────
exports.getMeals = async (req, res, next) => {
  try {
    const prisma = req.app.get('prisma');
    const { date } = req.query;

    // Default to today if no date provided
    const targetDate = date ? new Date(date) : new Date();
    const start = new Date(targetDate); start.setHours(0,  0,  0, 0);
    const end   = new Date(targetDate); end.setHours(23, 59, 59, 999);

    const meals = await prisma.meal.findMany({
      where:   { userId: req.user.id, date: { gte: start, lte: end } },
      orderBy: { date: 'asc' },
    });

    // Totals
    const totals = meals.reduce(
      (acc, m) => ({
        calories: acc.calories + m.calories,
        protein:  acc.protein  + m.protein,
        carbs:    acc.carbs    + m.carbs,
        fat:      acc.fat      + m.fat,
      }),
      { calories: 0, protein: 0, carbs: 0, fat: 0 }
    );

    res.json({ success: true, data: meals, totals });
  } catch (err) { next(err); }
};

// ── GET /api/diet/meals/:id ───────────────────────────────────────────────────
exports.getMeal = async (req, res, next) => {
  try {
    const prisma = req.app.get('prisma');
    const meal = await prisma.meal.findFirst({
      where: { id: req.params.id, userId: req.user.id },
    });
    if (!meal) return res.status(404).json({ success: false, message: 'Meal not found.' });
    res.json({ success: true, data: meal });
  } catch (err) { next(err); }
};

// ── POST /api/diet/meals ──────────────────────────────────────────────────────
exports.createMeal = async (req, res, next) => {
  try {
    const prisma = req.app.get('prisma');
    const { name, date, calories, protein, carbs, fat, items } = req.body;

    const meal = await prisma.meal.create({
      data: {
        userId:   req.user.id,
        name,
        date:     date ? new Date(date) : new Date(),
        calories: parseInt(calories),
        protein:  parseFloat(protein),
        carbs:    parseFloat(carbs),
        fat:      parseFloat(fat),
        items:    items || [],
      },
    });

    res.status(201).json({ success: true, message: 'Meal logged.', data: meal });
  } catch (err) { next(err); }
};

// ── PATCH /api/diet/meals/:id ─────────────────────────────────────────────────
exports.updateMeal = async (req, res, next) => {
  try {
    const prisma = req.app.get('prisma');
    const meal = await prisma.meal.findFirst({ where: { id: req.params.id, userId: req.user.id } });
    if (!meal) return res.status(404).json({ success: false, message: 'Meal not found.' });

    const { name, calories, protein, carbs, fat, items } = req.body;
    const updated = await prisma.meal.update({
      where: { id: req.params.id },
      data: {
        ...(name     && { name }),
        ...(calories && { calories: parseInt(calories) }),
        ...(protein  && { protein:  parseFloat(protein) }),
        ...(carbs    && { carbs:    parseFloat(carbs) }),
        ...(fat      && { fat:      parseFloat(fat) }),
        ...(items    && { items }),
      },
    });

    res.json({ success: true, message: 'Meal updated.', data: updated });
  } catch (err) { next(err); }
};

// ── DELETE /api/diet/meals/:id ────────────────────────────────────────────────
exports.deleteMeal = async (req, res, next) => {
  try {
    const prisma = req.app.get('prisma');
    const meal = await prisma.meal.findFirst({ where: { id: req.params.id, userId: req.user.id } });
    if (!meal) return res.status(404).json({ success: false, message: 'Meal not found.' });

    await prisma.meal.delete({ where: { id: req.params.id } });
    res.json({ success: true, message: 'Meal deleted.' });
  } catch (err) { next(err); }
};

// ── GET /api/diet/summary ─────────────────────────────────────────────────────
// Weekly calorie + macro summary
exports.getSummary = async (req, res, next) => {
  try {
    const prisma = req.app.get('prisma');
    const { days = 7 } = req.query;

    const since = new Date();
    since.setDate(since.getDate() - parseInt(days));
    since.setHours(0, 0, 0, 0);

    const meals = await prisma.meal.findMany({
      where:   { userId: req.user.id, date: { gte: since } },
      orderBy: { date: 'asc' },
    });

    // Group by day
    const byDay = {};
    meals.forEach(m => {
      const dayKey = m.date.toISOString().split('T')[0];
      if (!byDay[dayKey]) byDay[dayKey] = { calories: 0, protein: 0, carbs: 0, fat: 0, meals: 0 };
      byDay[dayKey].calories += m.calories;
      byDay[dayKey].protein  += m.protein;
      byDay[dayKey].carbs    += m.carbs;
      byDay[dayKey].fat      += m.fat;
      byDay[dayKey].meals    += 1;
    });

    const dailySummary = Object.entries(byDay).map(([date, data]) => ({
      date,
      day: new Date(date).toLocaleDateString('en-US', { weekday: 'short' }),
      ...data,
    }));

    // Averages
    const totalDays = dailySummary.length || 1;
    const avgCalories = Math.round(dailySummary.reduce((a, d) => a + d.calories, 0) / totalDays);
    const avgProtein  = Math.round(dailySummary.reduce((a, d) => a + d.protein,  0) / totalDays);

    res.json({
      success: true,
      data: {
        dailySummary,
        averages: { calories: avgCalories, protein: avgProtein },
        totalMealsLogged: meals.length,
      },
    });
  } catch (err) { next(err); }
};

// ── GET /api/diet/goals ───────────────────────────────────────────────────────
// Returns macro goals based on user profile + fitness goal
exports.getMacroGoals = async (req, res, next) => {
  try {
    const prisma = req.app.get('prisma');
    const user = await prisma.user.findUnique({ where: { id: req.user.id } });

    // Basic macro calculation
    const w = user.weight || 75;
    const goals = calculateMacroGoals(w, user.goal);

    res.json({ success: true, data: goals });
  } catch (err) { next(err); }
};

function calculateMacroGoals(weightKg, goal) {
  // Protein: 1.8-2.2g per kg based on goal
  // Calories: TDEE estimate (simplified)
  const baseTDEE = weightKg * 33; // moderate activity

  const configs = {
    MUSCLE_GAIN:      { calMult: 1.1,  proteinPerKg: 2.2, carbPct: 0.45, fatPct: 0.25 },
    FAT_LOSS:         { calMult: 0.8,  proteinPerKg: 2.4, carbPct: 0.30, fatPct: 0.30 },
    STRENGTH:         { calMult: 1.05, proteinPerKg: 2.0, carbPct: 0.40, fatPct: 0.30 },
    ENDURANCE:        { calMult: 1.15, proteinPerKg: 1.6, carbPct: 0.55, fatPct: 0.25 },
    GENERAL_FITNESS:  { calMult: 1.0,  proteinPerKg: 1.8, carbPct: 0.45, fatPct: 0.30 },
  };

  const cfg      = configs[goal] || configs.GENERAL_FITNESS;
  const calories = Math.round(baseTDEE * cfg.calMult);
  const protein  = Math.round(weightKg * cfg.proteinPerKg);
  const fat      = Math.round((calories * cfg.fatPct) / 9);
  const carbs    = Math.round((calories - protein * 4 - fat * 9) / 4);

  return { calories, protein, carbs, fat };
}
